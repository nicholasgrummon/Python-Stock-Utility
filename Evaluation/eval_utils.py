...
def moving_avg(data_history, period):
    '''
    Determines mean of data in window for each window of width=period in data_history
    ...
    '''
    mavg = []
    for i in range(period, len(data_history)):
        mavg.append(sum(data_history[i-period:i])/period)
    
    return mavg

def fibonacci_replacement_lvl(price):
    '''
    ...
    '''
    levels = np.array([0.236, 0.382, 0.500, 0.618, 0.786])    # ratios of subsequent fibonacci numbers
    return np.multiply(levels, price)
...
# def relative_strength_index(data_history, period, rsi_history=None):
#     '''
#     Determines the RSI momentum indicator given a data_history
...
def update_indicators(interval, ticker, dirFilepath):
    ...
    # history = pd.read_csv(f"{dirFilepath}/Historical/{interval}_history/{ticker}.csv", index_col=False)
...
update_indicators("1d","AAPL","/home/ncg/Documents/pythonStockUtility")